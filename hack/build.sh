...
export TAG=${TAG:-'v3.6.0'}
export CSV_PATH=${CSV_PATH:-'bundle/upstream/manifests/quay-bridge-operator.clusterserviceversion.yaml'}
...
docker buildx build --push --platform "linux/amd64,linux/ppc64le"  -t "${REGISTRY}/${NAMESPACE}/quay-bridge-operator:${TAG}" .
digest "${REGISTRY}/${NAMESPACE}/quay-bridge-operator:${TAG}" OPERATOR_DIGEST
...
make bundle IMG=${OPERATOR_DIGEST}
docker buildx build --push -f bundle.Dockerfile --platform "linux/amd64,linux/ppc64le"  -t "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle:${TAG}" .
digest "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle:${TAG}" BUNDLE_DIGEST

AMD64_DIGEST=$(skopeo inspect --raw  docker://${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle:${TAG} | \
               jq -r '.manifests[] | select(.platform.architecture == "amd64" and .platform.os == "linux").digest')
POWER_DIGEST=$(skopeo inspect --raw  docker://${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle:${TAG} | \
               jq -r '.manifests[] | select(.platform.architecture == "ppc64le" and .platform.os == "linux").digest')
        
opm index add --build-tool docker --bundles "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle@${AMD64_DIGEST}" --tag "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-amd64" --binary-image "quay.io/operator-framework/opm:v1.28.0-amd64"
docker push "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-amd64"
opm index add --build-tool docker --bundles "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-bundle@${POWER_DIGEST}" --tag "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-ppc64le" --binary-image "quay.io/operator-framework/opm:v1.28.0-ppc64le"
docker push "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-ppc64le"

docker manifest create --amend "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}" \
	"${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-amd64" \
	"${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}-ppc64le"
docker manifest push "${REGISTRY}/${NAMESPACE}/quay-bridge-operator-index:${TAG}"
